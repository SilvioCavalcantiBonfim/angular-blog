...
export class DashboardComponent implements AfterViewInit {
  ...
  publishIsBlock$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false,
  );
  ...
  constructor(
    ...
    private apiCommunication: APICommunicationService,
  ...
  updateArticleList() {
    this.flowList$
      .pipe(switchMap(() => this.apiCommunication.GetArticles()))
  ...
  updateProfile(a: {full_name?: string, email?: string, password?: string}) {
    this.apiCommunication.updateProfile(a).subscribe({
      next: (r) => (this.profile = r),
      error: () => this.permissions.logout(),
      complete: () =>
  ...
  publish() {
    combineLatest([
      ...
      this.publishIsBlock$.pipe(
        filter((v) => !v),
        switchMap(() => {
          if (this.editor) return this.editor?.update;
          else return of({ id: '', title: '', content: '', thumb: '' });
        }),
      ),
    ])
      .pipe(take(1))
      .subscribe({
        next: (input) => {
          ...
          if (Object.values(data).indexOf('') !== -1) {
            this.notification.add(
              'RED',
              'Algumas informações obrigatórias não foram preenchidas.',
              'newspaper',
              5000,
            );
            return;
          ...
          if (id) {
            this.apiCommunication.updateArticle(id, data).subscribe({
              ...
              complete: () => {
                ...
                this.notification.add(
                  'BLUE',
                  'Artigo foi atualizado com sucesso.',
                  'public',
                  5000,
                );
              },
